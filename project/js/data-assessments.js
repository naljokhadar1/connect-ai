...
(function () {
  ...
  const PROVIDER_BY = Object.fromEntries(PROVIDERS.map(p => [p.id, p]));

  // candidates for the send/assign flow (lighter than full pipeline records)
  const SEND_POOL = (window.DATA ? window.DATA.candidates : []).map(c => ({
  ...
  window.ASSESS = { QTYPES, TYPES, DOMAINS, DIFF, ASSESSMENTS, SEND_POOL, STATUS, sampleQ, buildResult, PROVIDERS, PROVIDER_BY };
})();
